...
data_formatada = data_atual.strftime('%d/%m/%Y às %H:%M:%S')
arquivo = open("resultado.txt", "w")
print(f"Sequência de testes emitidos em {data_formatada}",file=arquivo)
print('\n', file=arquivo)
...
for i, sequence in enumerate(attack_sequences):
    ...
    for j in range(len(sequence) - n + 1):
        ...
        if len(detected_subsequences) > 0:
            print(f"Sequência {i+1}",file=arquivo)
            print(f"Ataque detectado na sequência: {subsequence}", file=arquivo)
            ...
            for subsequence in detected_subsequences:
                ...
                sequence_str = sequence_str.replace(
                    subseq_str, f"//*{subseq_str}*//")
            print(sequence_str,file=arquivo)
            print('\n',file=arquivo)
            break  # interrompe a busca assim que uma sequência é detectada
    else:  # se o loop for terminar sem ter encontrado um ataque
        print(f"Sequência {i+1}",file=arquivo)
        print("Ataque não detectado na sequência:",file=arquivo)
        # converter a sequência em string
        sequence_str = ' '.join(str(elem) for elem in sequence)
        print(sequence_str ,file=arquivo)
        print('\n', file=arquivo)

# Finalização do txt de resultados.
arquivo.close()